# spammer.py (phiên bản 2.2 - Lazy Initialization Fix)
import requests
import re
import random
import string
...
class Spammer:
    # ... dán toàn bộ nội dung lớp Spammer từ phiên bản 2.1 vào đây ...
    ...
    def _format_proxy(self, proxy_str):
        if not proxy_str or not proxy_str.startswith(('http://', 'https://')):
...
class SpamManager:
    ...
    def _fetch_app_check_token(self):
        try:
            ...
            print(f"[ERROR] Lỗi khi kết nối đến API lấy token: {e}")
            return None
    ...
    def _load_proxies(self):
        ...
        try:
            with open('proxy.txt', 'r') as f:
                ...
                if file_proxies: proxies.update(file_proxies)
        except FileNotFoundError: pass
        for url in self.PROXY_APIS:
            try:
                res = requests.get(url, timeout=15); res.raise_for_status()
                proxies.update(p for p in res.text.splitlines() if p.strip())
    ...
    def find_locket_uid(self, user_input: str) -> Optional[str]:
        ...
        try:
            res = requests.get(url_to_check,timeout=15,allow_redirects=True); res.raise_for_status()
            if m:=re.search(r'locket\.camera/invites/([a-zA-Z0-9]{28})',res.url): return m.group(1)
            if m:=re.search(r'window\.location\.href\s*=\s*"([^"]+)"',res.text):
                if m_js:=re.search(r'locket\.camera/invites/([a-zA-Z0-9]{28})', m.group(1)): return m_js.group(1)
        except: pass
        return None
    ...
    def start_spam_session(self, user_id: int, target: str, custom_name: str, num_threads: int, update_callback: Callable):
        ...
        # === NEW === Đảm bảo mọi thứ sẵn sàng TRƯỚC khi bắt đầu luồng
        ...
        current_proxies_count = self.proxy_queue.qsize()
        actual_threads = min(num_threads, current_proxies_count)
        
        stats = {'accounts':0, 'requests':0, 'failed':0, 'start_time':time.time(), 'proxies':current_proxies_count}

        def spam_loop():
            # (Phần này giữ nguyên logic)
        ...
        session_thread = threading.Thread(target=spam_loop, daemon=True).start()
    ...
    def _run_worker(self, target_uid, custom_name, stop_event, stats):
        ...
        while not stop_event.is_set():
            try:
                ...
                self.proxy_queue.put(proxy) # Trả proxy lại
