# bot.py (phiên bản 4.2 - Đã sửa SyntaxError)
import discord
...
# ==============================================================================
# 2. HELPER & UI - SỬA LỖI CÚ PHÁP TẠI ĐÂY
# ==============================================================================
...
class ActiveSpamView(ui.View):
    ...
    async def stop_spam(self, interaction: discord.Interaction, button: ui.Button):
        if spam_manager.stop_spam_session(interaction.user.id):
            ...
            await interaction.response.send_message("Không tìm thấy phiên spam.", ephemeral=True)
...
class MyBotClient(discord.Client):
    ...
    async def on_interaction(self, interaction: discord.Interaction):
        # Mặc định gọi hàm xử lý của tree trước
        try:
            await self.tree.on_error(interaction, NotImplementedError)
        except:
             # Xử lý các lỗi interaction khác nếu cần
            pass

        # Xử lý các nút bấm động
...
async def genkey(interaction: discord.Interaction, user: discord.User, days: int):
    ...
    try:
        ...
        try:
            await user.send(f"🎉 Bạn nhận được key `{key_info['key']}` (hiệu lực {days} ngày). Dùng `/start` để sử dụng.")
        except:
            await interaction.followup.send(f"⚠️ Không gửi DM được.", ephemeral=True)
...
async def delkey(interaction: discord.Interaction, key: str):
    ...
    if keygen.delete_key(key):
        await interaction.followup.send(f"✅ Key `{key}` đã được vô hiệu hóa.", ephemeral=True)
    else:
        await interaction.followup.send(f"❌ Không tìm thấy key `{key}`.", ephemeral=True)
