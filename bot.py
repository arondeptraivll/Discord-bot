# bot.py (phiên bản 4.0 - Chống Timeout Toàn Diện)
import discord
...
print("--- [LAUNCH] Bot đang khởi chạy, phiên bản 4.0 (Ổn định)... ---")
...
# ==============================================================================
# 2. CÁC LỚP UI (Views & Modals) - Tối ưu hóa
# ==============================================================================
...
class SpamConfigModal(ui.Modal, title='Cấu hình Spam'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True) # Trả lời ngay lập tức
...
class ActiveSpamView(ui.View):
    ...
    async def update_message(self, status, stats=None, message=None):
        ...
        if status == "running":
            ...
            try: await self.status_message.edit(embed=embed)
            ...
            try: await self.status_message.edit(embed=embed, view=final_view)
...
class MyBotClient(discord.Client):
    ...
    async def on_interaction(self, interaction: discord.Interaction):
        # Xử lý nút bấm động (tạo sau khi spam xong)
        custom_id = interaction.data.get("custom_id") if interaction.data else None
        if custom_id and custom_id.startswith("spam_again:"):
            key = custom_id.split(":", 1)[1]
            await interaction.response.defer(ephemeral=True, thinking=True)
            result = spam_manager.validate_license(key)
            if result.get("valid"):
                await interaction.followup.send(view=SpamControlView(key, result['key_info']), ephemeral=True)
            else:
                await interaction.followup.send("Key của bạn đã hết hạn hoặc không hợp lệ khi thử lại.", ephemeral=True)
...
# Lệnh phải được định nghĩa trên 'client.tree'
...
# Tất cả các lệnh khác cũng phải defer()
...
async def listkeys(interaction: discord.Interaction):
    ...
    if not keys: await interaction.followup.send("Không có key nào đang hoạt động.", ephemeral=True); return
