# bot.py ("Professional Tool" Edition)
import discord
...
# ==============================================================================
# ✨ CONFIG GIAO DIỆN AOV - "Professional Tool" Edition ✨
# ==============================================================================
class AOV_UI_CONFIG:
    # --- Màu sắc Chuyên nghiệp ---
    COLOR_BRAND = 0x3498db # Màu xanh dương hiện đại
    COLOR_SUCCESS = 0x2ecc71 # Xanh lá cây
    COLOR_ERROR = 0xe74c3c  # Đỏ
    COLOR_WAITING = 0xf39c12 # Vàng cam

    # --- Hình ảnh tĩnh - Thay bằng logo của bạn hoặc để trống ---
    THUMBNAIL_URL = "https://i.imgur.com/wA2tmI9.png" 
...
class SpamSetupModal(ui.Modal, title='🛠️ Cấu hình phiên Spam'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        status_embed = discord.Embed(title="🔄 Khởi động phiên spam...", description=f"**Target:** `{target}`\n**Username:** `{custom_name}`\n**Emoji:** {'Bật' if use_emojis else 'Tắt'}", color=discord.Color.orange())
...
# ==============================================================================
# 4. UI VÀ LOGIC MỚI CHO CHỨC NĂNG LIÊN QUÂN (/start1) - "Professional" Edition
# ==============================================================================

class AOVAccountDashboardView(ui.View):
    """View điều khiển tài khoản sau khi nhận: Sao chép và Đổi tài khoản."""
    def __init__(self, username: str, password: str):
        super().__init__(timeout=None)
        self.username = username
        self.password = password
        
    @ui.button(label='Sao chép Tên TK', style=discord.ButtonStyle.secondary, emoji=AOV_UI_CONFIG.EMOJI_COPY, custom_id="aov_copy_user_btn")
    async def copy_username_callback(self, interaction: discord.Interaction, button: ui.Button):
        await interaction.response.send_message(f"```{self.username}```", ephemeral=True)

    @ui.button(label='Sao chép Mật Khẩu', style=discord.ButtonStyle.secondary, emoji=AOV_UI_CONFIG.EMOJI_COPY, custom_id="aov_copy_pass_btn")
    ...
    @ui.button(label='Đổi Tài Khoản', style=discord.ButtonStyle.success, emoji=AOV_UI_CONFIG.EMOJI_CHANGE, row=1, custom_id='aov_persistent_change')
    async def change_account(self, interaction: discord.Interaction, button: ui.Button):
        ...
        self.username = new_account["username"]
        self.password = new_account["password"]
        
        embed = discord.Embed(
            description=f"✅ Tài khoản đã được làm mới cho **{interaction.user.display_name}**.",
            color=AOV_UI_CONFIG.COLOR_SUCCESS
        )
        embed.set_author(name="GemLogin | Bảng Điều Khiển Tài Khoản", icon_url=client.user.display_avatar.url)
        ...
        embed.set_footer(text=f"Cập nhật lúc: {datetime.datetime.now(datetime.timezone.utc).strftime('%H:%M:%S, %d/%m/%Y')} UTC")

        await interaction.message.edit(embed=embed, view=self)
        await interaction.followup.send("✅ Đã làm mới tài khoản thành công!", ephemeral=True)
...
class AOVKeyEntryModal(ui.Modal, title='Xác thực License Key'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        processing_embed = discord.Embed(
            description=f"{AOV_UI_CONFIG.EMOJI_GEAR} Đang xử lý yêu cầu của bạn, vui lòng chờ...", 
            color=AOV_UI_CONFIG.COLOR_WAITING
        )
        await self.original_message.edit(embed=processing_embed, view=None)
        ...
        if not result.get("valid"):
            ...
            return await self.original_message.edit(embed=error_embed)
        ...
        success_embed = discord.Embed(
            description=f"✅ Key `{key_value}` đã được kích hoạt thành công cho **{interaction.user.display_name}**.",
            color=AOV_UI_CONFIG.COLOR_SUCCESS
        )
        ...
        success_embed.set_footer(text="Sử dụng các nút bên dưới để quản lý tài khoản của bạn.")

        dashboard_view = AOVAccountDashboardView(username=account["username"], password=account["password"])
        await self.original_message.edit(embed=success_embed, view=dashboard_view)
        await interaction.followup.send("Hoàn tất!", ephemeral=True)
...
class AOVInitialView(ui.View):
    ...
    async def enter_aov_key(self, interaction: discord.Interaction, button: ui.Button):
        await interaction.response.send_modal(AOVKeyEntryModal(self.original_message))
        
    async def on_timeout(self):
        try:
            if self.original_message and self.original_message.embeds: 
                embed = self.original_message.embeds[0]
                embed.description = "Phiên làm việc đã hết hạn. Vui lòng dùng lệnh `/start1` để bắt đầu lại."
...
@client.event
async def on_ready():
    client.add_view(AOVAccountDashboardView(username="", password="")) 
    await tree.sync()
...
async def start1(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            f"> Để nhận tài khoản, bạn cần có một **License Key** hợp lệ. Mỗi key chỉ có thể kích hoạt một lần.\n"
            f"> Liên hệ Admin <@{ADMIN_USER_ID}> để được cấp key."
        ),
        color=AOV_UI_CONFIG.COLOR_BRAND
    )
    embed.set_thumbnail(url=AOV_UI_CONFIG.THUMBNAIL_URL)
...
async def genkey(interaction: discord.Interaction, user: discord.User, days: int):
    async def logic(inter): 
        key_info = keygen.add_key(days, user.id, inter.user.id)
        await inter.followup.send(f"✅ **Đã tạo key Locket!**\n\n**Người dùng:** {user.mention}\n**Hiệu lực:** {days} ngày\n**Key:** `{key_info['key']}`", ephemeral=True)
...
async def genkey1(interaction: discord.Interaction, user: discord.User, days: int = 1):
    async def logic(inter): 
        key_info = aov_keygen.add_key(days, user.id, inter.user.id)
        await inter.followup.send(f"✅ **Đã tạo key Liên Quân!**\n\n**Người dùng:** {user.mention}\n**Hiệu lực:** {days} ngày\n**Key:** `{key_info['key']}`", ephemeral=True)
    await admin_command_wrapper(interaction, logic)
